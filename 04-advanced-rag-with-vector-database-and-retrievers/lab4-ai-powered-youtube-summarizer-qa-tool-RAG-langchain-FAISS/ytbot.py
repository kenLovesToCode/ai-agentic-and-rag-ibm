...
def patch_gradio_starlette_template_response():
    """
    Make older Gradio releases compatible with newer Starlette releases.

    Older Gradio versions call:
        TemplateResponse(template_name, context)
    while newer Starlette expects:
    ...
    if getattr(original_template_response, "_gradio_starlette_compat", False):
        return
        ...
        if (
            ...
            try:
                return original_template_response(
                    self, request, template_name, context, *args[2:], **kwargs
                )
            except TypeError:
                pass
    ...
    compatible_template_response._gradio_starlette_compat = True
    Jinja2Templates.TemplateResponse = compatible_template_response


patch_gradio_starlette_template_response()
